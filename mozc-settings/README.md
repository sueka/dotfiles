...
`'` は、続く仮名をカタカナにします。ひらがなが存在し、かつカタカナに変換できない仮名にのみ適用できます。 `𛀁`, `𛅐` (U+1B150; HIRAGANA LETTER SMALL WI), `𛅑` (U+1B151; HIRAGANA LETTER SMALL WE), `𛅒` (U+1B152; HIRAGANA LETTER SMALL WO), `ゔ`, `ゕ`, `ゖ` の7種類です。

##### 捨て仮名化フラグ

`x` は、続く仮名を捨て仮名にします。 `ぁ`, `ぃ`, `ぅ`, `ぇ`, `ぉ`, `ゕ`, `ㇰ`, `ゖ`, `ㇱ`, `ㇲ`, `っ`, `ㇳ`, `ㇴ`, `ㇵ`, `ㇶ`, `ㇷ`, `ㇸ`, `ㇹ`, `ㇺ`, `ゃ`, `ゅ`, `ょ`, `ㇻ`, `ㇼ`, `ㇽ`, `ㇾ`, `ㇿ`, `ゎ`, `𛅐` (U+1B150; HIRAGANA LETTER SMALL WI), `𛅑` (U+1B151; HIRAGANA LETTER SMALL WE), `𛅒` (U+1B152; HIRAGANA LETTER SMALL WO), `𛅧` (U+1B167; KATAKANA LETTER SMALL N) の32種類です。

カタカナの捨て仮名には、ひらがなから（`F7` や `⌥ X` などで）変換できないものがあります。これらはカタカナ化フラグを使って直接入力します。 `𛅤` (U+1B164; KATAKANA LETTER SMALL WI), `𛅥` (U+1B165; KATAKANA LETTER SMALL WE), `𛅦` (U+1B166; KATAKANA LETTER SMALL WO), `ヵ`, `ヶ` の5種類です。
...
ヮ式合拗音のイ段、エ段、オ段は `𛅐` (U+1B150; HIRAGANA LETTER SMALL WI), `𛅑` (U+1B151; HIRAGANA LETTER SMALL WE), `𛅒` (U+1B152; HIRAGANA LETTER SMALL WO) , `𛅤` (U+1B164; KATAKANA LETTER SMALL WI), `𛅥` (U+1B165; KATAKANA LETTER SMALL WE), `𛅦` (U+1B166; KATAKANA LETTER SMALL WO) ではなく `ゐ`, `ゑ`, `を`, `ヰ`, `ヱ`, `ヲ` を出力します。

捨て仮名化フラグといくつかの子音部分列にはエイリアスが存在します。 `l`, `q`, `sh`, `ts`, `ch`, `f`, `ph`, `j`, `dz` の9種類です。
...
恣意的な子音も存在します。 `v`, `c`, `th`, `dh` の4種類です。

| 恣意的な子音 | 意味                                                                                                                        |
|:-------------|:----------------------------------------------------------------------------------------------------------------------------|
| `v`          | `[v]` のような子音を表します。 `w` が有声化したものとして体系化しており、ァ式合拗音 `vh` とヸャ式開拗音 `vy` が存在します。 |
...
`z` を使った組み合わせは `z-`, `z,`, `z.`, `z[`, `z]`, `z/`, `zh`, `zj`, `zk`, `zl`, `z'`, `z"`, `z!`, `z?`, `z0`, `z1`, `z2`, `z3`, `z4`, `z5`, `z6`, `z7`, `z8`, `z9` の24種類です。
